...
from aphrodite.config.multimodal import MMCacheType, MMEncoderTPMode, MultiModalConfig
...
if TYPE_CHECKING:
    ...
    import aphrodite.modeling.models as me_models
    ...
    _Backend = Any
    me_quant = LazyLoader("modeling", globals(), "aphrodite.quantization")
    me_models = LazyLoader("modeling", globals(), "aphrodite.modeling.models")
    LoadConfig = Any
    ParallelConfig = Any
...
class ModelConfig:
    ...
    """The type of model runner to use. Each Aphrodite instance only supports one
    model runner, even if the same model can be used for multiple types."""
    convert: ConvertOption = "auto"
    """Convert the model using adapters defined in
    [aphrodite.modeling.models.adapters][]. The most common use case is to
    ...
    WARNING: The Aphrodite engine may crash if incorrect shape of embeddings is passed.
    ...
    def compute_hash(self) -> str:
        ...
        try:
            ...
            hf_config_json = (
                json.dumps(
                    json_map_leaves(
                        lambda v: v.to_dict() if isinstance(v, PretrainedConfig) else str(v),
        ...
        if self.seed is None:
            ...
            if not envs.APHRODITE_ENABLE_V1_MULTIPROCESSING:
                logger.warning(
                    ...
                    "APHRODITE_ENABLE_V1_MULTIPROCESSING is set to False, this may "
    ...
    def architecture(self) -> str:
        """The architecture aphrodite actually used."""
    ...
    def _verify_tokenizer_mode(self) -> None:
        ...
        if tokenizer_mode not in get_args(TokenizerMode):
            raise ValueError(
                f"Unknown tokenizer mode: {self.tokenizer_mode}. Must be one of {get_args(TokenizerMode)}."
            )
    ...
    def _verify_quantization(self) -> None:
        ...
        if self.quantization is not None and self.quantization in online_quant_methods:
            fp_bits = int(self.quantization[2])
            if fp_bits in [2, 3]:
                logger.warning(
                    "FP2 and FP3 quantization methods lead to "
    ...
    def verify_dual_chunk_attention_config(
        ...
        if hasattr(self.hf_config, "dual_chunk_attention_config"):
            ...
            from aphrodite.modeling.model_loader.weight_utils import get_sparse_attention_config
    ...
    def is_deepseek_mla(self) -> bool:
        if not hasattr(self.hf_text_config, "model_type"):
            ...
            return (
                self.hf_text_config.model.model_type in ("deepseek_v2", "deepseek_v3", "deepseek_v32")
    ...
    def get_layers_start_end_indices(self, parallel_config: ParallelConfig) -> tuple[int, int]:
        ...
        if (
            ...
            total_num_hidden_layers = getattr(self.hf_text_config, "num_nextn_predict_layers", 1)
        else:
    ...
    def try_get_generation_config(self) -> dict[str, Any]:
        ...
        """
        if self.generation_config in {"auto", "aphrodite"}:
    ...
    def get_diff_sampling_param(self) -> dict[str, Any]:
        ...
        - Aphrodite's neutral defaults if `self.generation_config="aphrodite"`
    ...
    def is_cross_encoder(self) -> bool:
        return self._model_info.supports_cross_encoding or self.convert_type == "classify"
...
def _get_and_verify_max_len(
    ...
    # NOTE: Gemma3's max_model_len (128K) is already scaled by RoPE
    ...
    # derived length from the HF model config.
        ...
        if model_max_length is not None and max_model_len <= model_max_length:
            ...
            warning = (
                "APHRODITE_ALLOW_LONG_MAX_MODEL_LEN must be used with extreme "
            ...
            if envs.APHRODITE_ALLOW_LONG_MAX_MODEL_LEN:
                logger.warning_once("%s %s", msg, warning)
            else:
                raise ValueError(
                    f"{msg} To allow overriding this maximum, set "
                    f"the env var APHRODITE_ALLOW_LONG_MAX_MODEL_LEN=1. {warning}"
                )
