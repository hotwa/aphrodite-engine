...
class CudaPlatformBase(Platform):
    ...
    def supported_dtypes(self) -> list[torch.dtype]:
        ...
        # though Aphrodite doesn't support these GPUs.
    ...
    def check_and_update_config(cls, aphrodite_config: "AphroditeConfig") -> None:
        ...
        if model_config is not None and model_config.use_mla and cache_config.block_size is not None:
            ...
            if use_flashinfer_mla and cache_config.block_size != 32 and cache_config.block_size % 64 != 0:
                cache_config.block_size = 64
                logger.info("Forcing kv cache block size to 64 for FlashInferMLA backend.")

            # TODO(Chen): remove this hacky code
            if use_sparse and cache_config.block_size != 64:
                cache_config.block_size = 64
                logger.info("Forcing kv cache block size to 64 for FlashMLASparse backend.")
    ...
    def get_attn_backend_cls(
        ...
        if use_mla:
            ...
            use_cutlassmla = selected_backend == _Backend.CUTLASS_MLA or (
                selected_backend is None and cls.is_device_capability(100) and block_size % 128 == 0
            )
            ...
            if use_flashmla:
                if block_size % 64 != 0:
                    logger.warning(
                        "FlashMLA backend is not supported for block size %d (currently only supports block size 64).",
                        block_size,
                    )
                else:
